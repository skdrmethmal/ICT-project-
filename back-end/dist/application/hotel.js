...
exports.Generate = exports.updateHotel = exports.deleteHotel = exports.addHotel = exports.getHotelById = exports.getAllHotels = void 0;
...
const updateExistingHotels = async () => {
    try {
        const hotels = await Hotel_1.default.find({ stripePriceId: { $exists: false } });
        hotels.forEach(async (hotel) => {
            if (!hotel.stripePriceId) {
                ...
                hotel.stripePriceId = stripeProduct.default_price;
                await hotel.save();
...
const getAllHotels = async (req, res, next) => {
    try {
        ...
        if (!hotels) {
            throw new not_found_error_1.default("Hotels not found");
        }
...
const addHotel = async (req, res, next) => {
    try {
        const newHotel = hotel_1.addHotelDTO.safeParse(req.body);
        console.log(req.body);
        if (!newHotel.success) {
            throw new validation_error_1.default("Hotel data is not valid");
        }
        // if (
        //   !newHotel.name ||
        //   !newHotel.location ||
        ...
        const savedHotel = await Hotel_1.default.create({
            name: newHotel.data.name,
        ...
        (0, embeddings_1.createEmbedding)(savedHotel);
...
const deleteHotel = async (req, res, next) => {
    try {
        ...
        const hotel = await Hotel_1.default.findById(hotelId);
        if (!hotel) {
            throw new not_found_error_1.default("Hotel to delete not found");
        }
...
const updateHotel = async (req, res, next) => {
    try {
        ...
        const hotel = await Hotel_1.default.findById(hotelId);
        if (!hotel) {
            throw new not_found_error_1.default("Hotel to update not found");
        }
        if (!updateHotel.name ||
            ...
            throw new validation_error_1.default("Please provide all the required fields");
        }
        await Hotel_1.default.findByIdAndUpdate(hotelId, updateHotel);
